    ...
    <div style={{ fontFamily:'Nunito,sans-serif' }}>
      ...
      <div className="grid-stats-auto" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16, marginBottom:24 }}>
        {[
          ...
          { label:'Inactive',         value:warehouses.filter(w=>w.status!=='active').length, icon:'ri-close-circle-line', color:'var(--text-light)' },
        ].map(c => (
          <div key={c.label} style={{ background:'var(--bg-card)', borderRadius:12, border:'1px solid var(--border)', borderLeft:`3px solid ${c.color}`, padding:'16px 20px', display:'flex', alignItems:'center', gap:12, boxShadow:'0 1px 4px rgba(0,0,0,0.06)' }}>
            ...
            <div key={w.id} style={{ background:'var(--bg-card)', borderRadius:14, border:'1px solid var(--border)', boxShadow:'0 1px 4px rgba(0,0,0,0.06)', overflow:'hidden' }}>
              ...
              <div style={{ padding:20 }}>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12 }}>
                  <div>
                    <div style={{ fontSize:11, color:'var(--text-light)', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:4 }}>Location</div>
                    <div style={{ fontSize:13, color:'var(--text-secondary)', fontWeight:600 }}><i className="ri-map-pin-line" style={{ marginRight:4, color:'var(--text-muted)' }}/>{w.location || '—'}</div>
                  </div>
                  <div>
                    <div style={{ fontSize:11, color:'var(--text-light)', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:4 }}>Manager</div>
                    <div style={{ fontSize:13, color:'var(--text-secondary)', fontWeight:600 }}><i className="ri-user-line" style={{ marginRight:4, color:'var(--text-muted)' }}/>{w.manager || '—'}</div>
                  </div>
                </div>

                {w.capacity > 0 && (
                  <div style={{ background:'var(--bg-subtle)', borderRadius:8, padding:'10px 14px', display:'flex', justifyContent:'space-between', alignItems:'center', fontSize:12 }}>
                    <span style={{ color:'var(--text-muted)' }}>Capacity</span>
                    <span style={{ fontWeight:700, color:'var(--text-secondary)' }}>{Number(w.capacity).toLocaleString()} units</span>
                  </div>
                )}
