  ...
  const FormContent = () => (
    <div style={{ padding:24, overflowY:'auto' }}>
      <div className="grid-form-cols" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
        ...
        <div style={{ gridColumn:'1 / -1' }}>
          <label style={lbl}>Coverage Areas <span style={{ fontWeight:400, color:'var(--text-light)' }}>(comma-separated)</span></label>
        ...
        <div style={{ gridColumn:'1 / -1' }}>
          ...
          <div style={{ border:'1px solid var(--border)', borderRadius:8, padding:'12px 14px' }}>
            {allDrivers.length===0
              ? <div style={{ fontSize:12, color:'var(--text-light)' }}>No drivers available</div>
              : allDrivers.map(d => (
                <label key={d.id} style={{ display:'flex', alignItems:'center', gap:8, cursor:'pointer', fontSize:12, color:'var(--text-secondary)', marginBottom:6 }}>
  ...
  return (
    <div style={{ fontFamily:'Nunito, sans-serif' }}>
      ...
      <div style={{ ...card, padding:'10px 14px', marginBottom:14, display:'flex', flexWrap:'wrap', gap:10, alignItems:'center' }}>
        ...
        <div style={{ marginLeft:'auto', fontSize:12, color:'var(--text-muted)' }}>{filtered.length} zone{filtered.length!==1?'s':''}</div>
      </div>
            ...
            <div key={zone.id} style={{ ...card, display:'flex', flexDirection:'column', borderTop:`3px solid ${activeColor}` }}>
              <div style={{ padding:'16px', flex:1 }}>
                ...
                <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:14 }}>
                  <div>
                    <div style={{ fontWeight:700, fontSize:15, color:'var(--text-primary)' }}>{zone.zone_name}</div>
                    <div style={{ display:'flex', alignItems:'center', gap:8, marginTop:4 }}>
                      ...
                      <span style={{ fontSize:11, color:'var(--text-muted)' }}><i className="ri-time-line" style={{ marginRight:3 }} />{zone.estimated_eta||'—'}</span>
                    </div>
                  </div>
                  {/* Dropdown menu */}
                  <div style={{ position:'relative' }}>
                    ...
                    {menuZone===zone.id && (
                      <>
                        ...
                        <div style={{ position:'absolute', right:0, top:'100%', zIndex:50, background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:8, boxShadow:'0 4px 16px rgba(0,0,0,0.12)', minWidth:160, overflow:'hidden', marginTop:4 }}>
                          {[
                ...
                <div style={{ display:'flex', gap:8, marginBottom:14 }}>
                  {[
                    ...
                    <div key={b.label} style={{ flex:1, border:'1px solid var(--border)', borderRadius:8, padding:'8px 6px', textAlign:'center' }}>
                      <div style={{ fontWeight:700, fontSize:14, color:b.color }}>{b.val}</div>
                      <div style={{ fontSize:10, color:'var(--text-light)' }}>{b.label}</div>
                    </div>
                  ))}
                </div>
                ...
                {/* Drivers */}
                <div style={{ marginBottom:12 }}>
                  <div style={{ fontSize:11, color:'var(--text-light)', marginBottom:5 }}>Assigned Drivers</div>
                  {zoneDrivers.length===0
                    ...
                    : <div style={{ display:'flex', flexWrap:'wrap', gap:5 }}>
                        {zoneDrivers.map(d => (
                          <div key={d.id} style={{ display:'flex', alignItems:'center', gap:5, border:'1px solid var(--border)', borderRadius:50, padding:'2px 8px', background:(DRIVER_STATUS_COLOR[d.status]||'#6b7280')+'10', fontSize:11 }}>
            ...
            return (
              <ModalShell title={selected.zone_name} onClose={closeModal} maxWidth={520}>
                <div style={{ overflowY:'auto' }}>
                  <div style={{ background:'#f0fdf4', padding:'12px 20px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', gap:10 }}>
                  ...
                  <div style={{ padding:20 }}>
                    ...
                    <div style={{ marginBottom:16 }}>
                      ...
                      {zoneDrivers.length===0
                        ...
                        : zoneDrivers.map(d => (
                          <div key={d.id} style={{ display:'flex', alignItems:'center', gap:10, border:'1px solid var(--border)', borderRadius:8, padding:'10px 12px', marginBottom:6 }}>
