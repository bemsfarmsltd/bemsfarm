...
Last verified: 2026-07-30, against `server/src/index.js` directly (the mount list below is
...
| API | `server/` (Express, entry `server/src/index.js`) | Postgres (Supabase) | CommonJS, no build step. `npm start` / `npm run dev` (nodemon) from `server/`. |
